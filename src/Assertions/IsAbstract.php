...
class IsAbstract implements Assertable
{
    public function assert(PendingFile $file, bool $negate = false, array $except = []): void
    {
        ...
        collect((new NodeFinder())->findInstanceOf($ast, Class_::class))
            ...
            ->filter(fn(Class_ $class) => $class->isAbstract())
            ->when(fn(Collection $collection) => ($collection->isNotEmpty() && count($except) > 0) || count($except) === 0, fn(Collection $collection) => $collection
            ->when(
                !$negate,
                fn(Collection $nodes) => $nodes->whenEmpty(fn() => Assert::fail("{$file->localPath} is not an abstract class")),
                fn(Collection $nodes) => $nodes->whenNotEmpty(fn() => Assert::fail("{$file->localPath} is an abstract class")),
            )
            );
